...
std::string EverythingConstructionExecutor::ConstructionName() const {
    return "Everything";
}
...
DPA EverythingConstructionExecutor::PerformConstruction(const BaseOptions& options_) const {
    DPA dpa = *this->in_dpa;
    ...
    EquivalenceRelation<state_t> language_equivalence = LanguageEquivalentStates(dpa);
...
std::unique_ptr<ConstructionExecutor> CreateConstructionExecutor() {
    return std::make_unique<EverythingConstructionExecutor>();
}