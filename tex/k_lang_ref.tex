...
\begin{lem}
	For all $p, q \in \lambda$ and $k \in c(Q)$: $(\iota_p^k, \iota_q^k) \in V$.
\end{lem}
...
\begin{lem}
	Let $\mathcal{A}$, $R$, and $\lambda$ be as before. For every $q \in \lambda$, $l, k \in c(Q)$, and $w \in \Sigma^+$: $(\delta^\lambda_\text{visit})^*(\iota_q^k, w) = (\delta^\lambda_\text{visit})^*(\iota_q^l, w)$.
\end{lem}

\begin{proof}
	We provide a proof via induction on the word length $|w|$. 
	
	If $w = a \in \Sigma$, we have $\delta^\lambda_\text{visit})^*(\iota_q^k, w) = \delta^\lambda_\text{visit}(\iota_q^k, a) = (\delta(q, a), \min \{c(q), c(q)\}) = (\delta(q, a), c(q))$. Analogously, $\delta^\lambda_\text{visit})^*(\iota_q^k, w) = (\delta(q, a), c(q))$, which is what had to be shown.
	
	Now consider $w = ua \in \Sigma^* \cdot \Sigma$. We have to show that $(\delta^\lambda_\text{visit})^*(\iota_q^k, ua) = (\delta^\lambda_\text{visit})^*(\iota_q^l, ua)$. By induction, there is a state $x \in Q^\lambda_\text{visit}$ with $(\delta^\lambda_\text{visit})^*(\iota_q^k, u) = (\delta^\lambda_\text{visit})^*(\iota_q^l, u) = x$. Then $(\delta^\lambda_\text{visit})^*(\iota_q^k, ua) = (\delta^\lambda_\text{visit})^*(\iota_q^l, ua) = \delta^\lambda_\text{visit}(x, a)$.
\end{proof}

\begin{lem}
	Let $\mathcal{A}$, $R$, and $\lambda$ be as before. For all $p, q \in \lambda$, and $l, k \in c(Q)$, $(\iota_p^k, \iota_q^k) \in V_M$ if and only if $(\iota_p^l, \iota_q^l) \in V_M$.
\end{lem}
...
\begin{proof}
	...
	It must be true that $w \neq \varepsilon$; otherwise, $(\iota_p^l, \iota_q^l) \notin V$, which would contradict Lemma \ref{}.
	
	By Lemma \ref{}, we have $(\delta^\lambda_\text{visit})^*(\iota_p^l, w) = (\delta^\lambda_\text{visit})^*(\iota_p^k, w)$ and analogously for $q$. Therefore, $((\delta^\lambda_\text{visit})^*(\iota_p^k, w), (\delta^\lambda_\text{visit})^*(\iota_q^k, w)) \notin V$ and thus $(\iota_p^k, \iota_q^k) \notin V_M$, which contradicts our assumption.
\end{proof}

\begin{lem}
	Let $q \in \lambda$, $k \in c(Q)$, $w \in L_{\lambda \hookleftarrow}$, and $\varepsilon \sqsubset v \sqsubset w$. Then Then $(\delta^\lambda_\text{visit})^*(\iota_q^k, v) = (\delta^*(q, v), x_v, \perp)$, where $x_v = \min \{c(\delta^*(q, u)) \mid u \sqsubseteq v\}$.
\end{lem}

\begin{proof}
We provide this claim by induction on $v$. First, consider $v = a \in \Sigma$, so $(\delta^\lambda_\text{visit})^*(\iota_q^k, v) = \delta^\lambda_\text{visit}(\iota_q^k, a) = (\delta(q, a), \min \{c(q), c(\delta(q, a))\}, \perp)$. This is exactly what we had to show, as $\min \{c(q), c(\delta(q, a))\} = x_a$.
	
	For the induction step, let $v = v'a \in \Sigma^+ \cdot \Sigma$. Then $(\delta^\lambda_\text{visit})^*(\iota_q^k, v) = \delta^\lambda_\text{visit}((\delta^*(q, v'), x_{v'}, \perp), a)$. Definition tells us that 
\end{proof}
...
\begin{proof}
	For all $v \sqsubseteq w$, let $\min \{c(\delta^*(q, u)) \mid u \sqsubseteq v\}$ (i.e. $x = x_w$. Let $w = va \in \Sigma^* \cdot \Sigma$ (since words in $L_{\lambda \hookleftarrow}$ are non-empty). Then $(\delta^\lambda_\text{visit})^*(\iota_q^k, v) = (\delta^*(q, v), x_v, \perp)$ by Lemma \ref{} and $(\delta^\lambda_\text{visit})^*(\iota_q^k, w) = \delta^\lambda_\text{visit}((\delta^*(q, v), x_v, \perp), a)$.
	
	Since $\delta(\delta^*(q, v), a) = \delta^*(q, w)) \in \lambda$, definition tells us $\delta^\lambda_\text{visit}((\delta^*(q, v), x_v, \perp), a) = (\delta^*(q, w), c(\delta^*(q, w)), \min \{x_v, c(\delta^*(q, w)\})$. The fact that $\min \{x_v, c(\delta^*(q, w)\} = x_w$ finishes our proof. 
\end{proof}
...
\begin{proof}
	...
	If $n = 0$, there is a word $w \in L_{\lambda \hookleftarrow}$ such that $\min \{c(\delta^*(p, u)) \mid u \sqsubseteq w\} \neq \min \{c(\delta^*(q, u)) \mid u \sqsubseteq w\}$. Let $x, y \in c(Q)$ be the third components of $(\delta^\lambda_\text{visit})^*(\iota_p^{\hat{c}}, w)$ and $(\delta^\lambda_\text{visit})^*(\iota_q^{\hat{c}}, w)$ respectively. By Lemma \ref{}, $x = \min \{c(\delta^*(p, u)) \mid u \sqsubseteq w\}$ and $y = \min \{c(\delta^*(q, u)) \mid u \sqsubseteq w\}$, so $x \neq y$. By definition of $\mathcal{A}^\lambda_\text{visit}$, this means that $((\delta^\lambda_\text{visit})^*(\iota_p^{\hat{c}}, w), (\delta^\lambda_\text{visit})^*(\iota_q^{\hat{c}}, w)) \notin V$.
	
	For $n+1 > 0$, there is a $w \in L_{\lambda \hookleftarrow}$ such that $(\delta^*(p, w), \delta^*(q, w)) \notin V$. Let $p' = \delta^*(p, w)$ and $q'$ analogously. By induction, $(\iota_{p'}^{\hat{c}}, \iota_{q'}^{\hat{c}}) \notin V_M$. Lemma \ref{} tells us that there are $k'$ and $l'$ such that $(\delta^\lambda_\text{visit})^*(\iota_p^{\hat{c}}, w) = \iota_{p'}^{k'}$ and $(\delta^\lambda_\text{visit})^*(\iota_q^{\hat{c}}, w) = \iota_{q'}^{l'}$. Since $n$ was chosen to be minimal, it must be true that $k' = l'$; otherwise, we would already have $p, q \notin X_0$. From Lemma \ref{}, we know that $(\iota_{p'}^{\hat{c}}, \iota_{q'}^{\hat{c}}) \notin V_M$ if and only if $(\iota_{p'}^{k'}, \iota_{q'}^{l'}) \notin V_M$, which is false. Thus, finally, $(\iota_p^{\hat{c}}, \iota_q^{\hat{c}}) \notin V_M$.
\end{proof}
