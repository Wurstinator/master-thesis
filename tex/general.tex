...
\begin{lem}
\label{lem:general:congrel_prio_implies_moore1}
	Let $\mathcal{A} = (Q, \Sigma, \delta, c)$ be a DPA and let $\sim$ be a congruence relation such that $p \sim q$ implies $c(p) = c(q)$. Let $\mathcal{A}'$ be a representative merge of $\mathcal{A}$ w.r.t. $\mu_\div^\sim$. For all $q \in Q'$, $(\mathcal{A}, q) \equiv_M (\mathcal{A}', q)$.
\end{lem}
...
\begin{lem}
\label{lem:general:congrel_prio_implies_moore2}
